...
```{r 1.0}
shva <- read.csv("https://raw.githubusercontent.com/agricolamz/2018-MAG_R_course/master/data/duryagin_ReductionRussian.txt", sep = "\t", encoding = "UTF-8")
```
...
Design it to look likegg [this](https://raw.githubusercontent.com/agricolamz/2018-MAG_R_course/master/img/duryagin2.png) and [this](https://raw.githubusercontent.com/agricolamz/2018-MAG_R_course/master/img/duryagin3.png).
```{r 1.2}
# f1 boxplot
ggplot(shva, aes(x = f1, y = vowel, fill = vowel)) +
      geom_boxplot()+
  labs(title = "f1 distribution in each vowel")
# f2 boxplot
ggplot(shva, aes(x = f2, y = vowel, fill = vowel)) +
      geom_boxplot()+
...
```{r 1.4}
aaa <- subset(shva, shva$vowel == "a")
yyy <- subset(shva, shva$vowel == "y")
AAA <- subset(shva, shva$vowel == "A")
cor(aaa$f1, aaa$f2)
cor(yyy$f1, yyy$f2)
cor(AAA$f1, AAA$f2)
...
```{r 2.2.1}
summary(lm(data = shva, f2~f1+vowel))
```
...
```
f2 depends on vowel "y" changes in f1, vowel "A" is insignificant. 
```
...
```{r 3.1}
If we want to have a statistical significance we need to perform a Fisher's Exact test on categorical variables with 2 values.In the other cases Pearson's Chi-squared test should be used.
```

### 3.2. 
Test the hypothesis that `Aux` and `EPTrans` are not independent with the help of Pearson's Chi-squared Test. 
```{r 3.2}
chisq <- chisq.test(d_caus$Aux, d_caus$EPTrans)
```
...
```{r 3.4}
oddsratio(chisq$observed, log = F)
...
```{r 3.4.1}
oddsratio(chisq$expected, log = F)
```
What can you say about odds ratio of expected frequencies for arbitrary data?
```
Odds ratio are used to meausre the strength of association of expected frequencies, as we can see it's equals 1, so there's no association.
```
### 3.5 
Calculate effect size for this test using Cramer's V (phi).
```{r 3.5}
sqrt(chisq$statistic/(length(d_caus$Aux) * (min(length(unique(d_caus$Aux)),length(unique(d_caus$EPTrans)))-1)))
```

### 3.6. 
Report the results of independence test using the following template:
```
We have found a significant association between variables Aux and EPTrans(p < 0.001).  The odds of intransitive predicate were 2.6 times higher in (group doen) than in (group laten). Effect size is small (Cramer's V = 0.169).
```
...
```{r 3.7}
mosaic(~ Aux + EPTrans, data = d_caus, shade = T, legend = T)
```
...
```
If we want a correct answer we need to adjust for the multiplicity, chisq test has a probability of statistical error.
```

### 3.9 
Provide a short text (300 words) describing the hypothesis of this study and the results of your analysis.
```{r 3.9}
The hypothesis of this study is about having a relation between auxiliaries "doen"/"laten" and the transitivity/intransitivity of the effected predicate. We found a significant association between our two categorical variables, "laten" is used much often with intransitive predicate than "doen" (2.6 times).We can also state that effect size is small.
```
